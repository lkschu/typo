...
border_chars = namedtuple("border_chars", ["left", "right", "top", "bottom", "topleft", "topright", "bottomleft", "bottomright"])
...
def fix_height(char_buffer:List[List[str]],focus_line:int,height:int,replace_top:List[str]=None,replace_bottom:List[str]=None)-> List[List[str]]:
...
class Session():
    ...
    def draw_session(self):
        ...
        if subwin_width <40:
            raise ValueError(f"Window to small to start session: {subwin_height,subwin_width}")
        self.sessionscreen = self.screen.subwin(subwin_height,subwin_width,top,left,)
        ...
        wpm_y = WPM_WINDOW.window_spacing.top if WPM_WINDOW.window_spacing.top is not None else self.screen.getmaxyx()[0]-WPM_WINDOW.nlines -WPM_WINDOW.window_spacing.bottom
        wpm_x = WPM_WINDOW.window_spacing.left if WPM_WINDOW.window_spacing.left is not None else self.screen.getmaxyx()[1]-WPM_WINDOW.ncols -WPM_WINDOW.window_spacing.right
        acc_y = ACC_WINDOW.window_spacing.top if ACC_WINDOW.window_spacing.top is not None else self.screen.getmaxyx()[0]-ACC_WINDOW.nlines -ACC_WINDOW.window_spacing.bottom
        acc_x = ACC_WINDOW.window_spacing.left if ACC_WINDOW.window_spacing.left is not None else self.screen.getmaxyx()[1]-ACC_WINDOW.ncols -ACC_WINDOW.window_spacing.right
    ...
    def draw_characters(self):
        ...
        c = len([x for x in typed[line] if x != ""])
        ...
        guide_chars = fix_height(self.text.get_guide_chars(width=x-2),focus_line=line,height=y-2,replace_top=[""],replace_bottom=["v","v","v"])
        for i,l in enumerate(guide_chars):
            self.sessionscreen.addstr(i+1,1,"".join(l))
        # print correct chars
        correct_chars = fix_height(self.text.get_typed_chars(width=x-2,correct=True),focus_line=line,height=y-2,replace_top=["^","^","^"])
        for iy,l in enumerate(correct_chars):
        ...
        for iy,l in enumerate(typo_chars):
            for ix,c in enumerate(l):
                if c:
                    if iy+1 > curs_y:
                        curs_y = iy+1
